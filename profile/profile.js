		...
		if(li.querySelector("span.isPay").innerText === "true"){
			li.querySelector("span.isPay").innerHTML = `
				...
				<div class="order-info">
					<span>已支付</span>
					<button class="order-del" disabled style="cursor: no-drop;">删除订单</button>
	...
	if(document.querySelector("span.name>em")){
		document.querySelector("span.name>em").onclick = function(){
			...
			Message.confirm("是否退出登录？", function(){
				...
				// 先不能删 删除的话当用户退出 不关浏览器 再次登录的时候 订单就没了 解决办法：先让他隐藏下
				// orderList = orderList.splice(0, orderList.length);
				document.querySelector("ul.order-list-show").style.display = "none";
				sessionStorage.setItem("data", JSON.stringify(data));
				window.location.href = window.location.href;
				// document.querySelector("a.addressManager").href = "../RegistLogin/login.html";
			})
